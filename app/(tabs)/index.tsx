...
type LibraryTab = 'history' | 'bookmarks' | 'downloads' | 'videos' | 'savedPages';
  ...
  return (
    <View style={[styles.screen, { backgroundColor: displayColors.background }]}>
      ...
      <Modal visible={libraryOpen} animationType="slide" transparent onRequestClose={() => setLibraryOpen(false)}>
        <View style={[styles.modalBackdrop, { backgroundColor: 'rgba(0,0,0,.52)' }]}>
          <View style={[styles.sheet, { backgroundColor: displayColors.background, paddingBottom: insets.bottom + 12 }]}>
            <View style={[styles.sheetHeader, { flexDirection: rowDirection }]}><Text style={[styles.sheetTitle, { color: displayColors.foreground }]}>{libraryTab === 'history' ? lang.history : libraryTab === 'bookmarks' ? lang.bookmarks  : libraryTab === 'downloads' ? lang.downloadManager : libraryTab === 'videos' ? 'فيديوهاتي' : 'الصفحات المحفوظة'}</Text><IconButton name="close" label={lang.close} color={displayColors.mutedForeground} onPress={() => setLibraryOpen(false)} /></View>
            <View style={[styles.segmented, { backgroundColor: displayColors.secondary, flexDirection: rowDirection }]}>
              {(['history', 'bookmarks', 'downloads', 'videos', 'savedPages'] as LibraryTab[]).map((value) => <Pressable key={value} onPress={() => setLibraryTab(value)} style={[styles.segment, libraryTab === value && { backgroundColor: displayColors.card }]}><Text style={[styles.segmentText, { color: libraryTab === value ? displayColors.primary : displayColors.mutedForeground }]}>{value === 'history' ? lang.history : value === 'bookmarks' ? lang.bookmarks  : value === 'downloads' ? lang.downloads : value === 'videos' ? 'فيديوهاتي' : 'محفوظة'}</Text></Pressable>)}
            </View>
