  ...
  return (
    <View style={[styles.screen, { backgroundColor: displayColors.background }]}>
      ...
      <Modal visible={libraryOpen} animationType="slide" transparent onRequestClose={() => setLibraryOpen(false)}>
        <View style={[styles.modalBackdrop, { backgroundColor: 'rgba(0,0,0,.52)' }]}>
          <View style={[styles.sheet, { backgroundColor: displayColors.background, paddingBottom: insets.bottom + 12 }]}>
            <View style={[styles.sheetHeader, { flexDirection: rowDirection }]}><Text style={[styles.sheetTitle, { color: displayColors.foreground }]}>{libraryTab === 'history' ? lang.history : libraryTab === 'bookmarks' ? lang.bookmarks : libraryTab === 'downloads' ? lang.downloadManager : 'فيديوهاتي'}</Text><IconButton name="close" label={lang.close} color={displayColors.mutedForeground} onPress={() => setLibraryOpen(false)} /></View>
            <View style={[styles.segmented, { backgroundColor: displayColors.secondary, flexDirection: rowDirection }]}>
              {(['history', 'bookmarks', 'downloads', 'videos'] as LibraryTab[]).map((value) => <Pressable key={value} onPress={() => setLibraryTab(value)} style={[styles.segment, libraryTab === value && { backgroundColor: displayColors.card }]}><Text style={[styles.segmentText, { color: libraryTab === value ? displayColors.primary : displayColors.mutedForeground }]}>{value === 'history' ? lang.history : value === 'bookmarks' ? lang.bookmarks : value === 'downloads' ? lang.downloads : 'فيديوهاتي'}</Text></Pressable>)}
            </View>
      ...
      <UCMiniTools visible={toolsOpen} onClose={()=>setToolsOpen(false)} nightMode={nightMode} textOnly={textOnly} onToggleNight={toggleNight} onToggleText={toggleText} onSavePage={saveCurrentPage} onOpenVideos={()=>{setToolsOpen(false);setLibraryTab('videos');setLibraryOpen(true)}} onOpenSavedPages={()=>{setToolsOpen(false);setNotice(`الصفحات المحفوظة: ${savedPages.length}`)}} bookmarks={bookmarks} setBookmarks={setBookmarks} onNotice={setNotice}/>
...
const styles = StyleSheet.create({
  ...
  internalPlayerScreen: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, zIndex: 9998, elevation: 30, backgroundColor: '#000000' },
  floatingPlayer: { position: 'absolute', right: 12, bottom: 78, width: 190, height: 116, zIndex: 9999, elevation: 40, borderRadius: 12, overflow: 'hidden', backgroundColor: '#000000', shadowOpacity: 0.35, shadowRadius: 10, shadowOffset: { width: 0, height: 4 } },
